...
public static class UIModule
{
    ...
    public static void Inject(MonoBehaviour owner, params object[] data)
    {
        ...
        if (owner.TryGetComponent<InjectionDestroyHook>(out var injectionDestroyHook))
        {
            injectionDestroyHook.OnDestroyed += Unject;
        }
        else
        {
            owner.gameObject.AddComponent<InjectionDestroyHook>().OnDestroyed += Unject;
        } 
    } 
